...
class RenderConfig:
    ...
    stationary_x_limit: float = 0.12
    stationary_y_spread: float = 0.28
...
def estimate_forward_only_motion(df: pd.DataFrame, y_offset: float, cfg: RenderConfig):
    ...
    a_lat = moving_savgol(acc_world[:, 1], 41, 3)
...
def estimate_stationary_motion(df: pd.DataFrame, y_offset: float, cfg: RenderConfig):
    ...
    절대 전진은 제거하되, 로컬 가속/자세 변화에서 온 작은 중심 이동을 남겨
    인솔 자체의 움직임이 더 자연스럽게 보이도록 한다.
    """
    tr = estimate_forward_only_motion(df, y_offset=y_offset, cfg=cfg)
    tr["pos"] = tr["pos"].copy()

    stance_f = moving_savgol(tr["stance"].astype(float), 31, 3)
    swing = np.clip(1.0 - stance_f, 0.0, 1.0)
    yaw = np.deg2rad(tr["rel_yaw_deg"])
    a_fwd = np.asarray(tr["a_fwd"], dtype=float)
    a_lat = np.asarray(tr.get("a_lat", np.zeros_like(a_fwd)), dtype=float)

    f_scale = np.quantile(np.abs(a_fwd), 0.95)
    l_scale = np.quantile(np.abs(a_lat), 0.95)
    f_scale = float(f_scale) if f_scale > 1e-8 else 1.0
    l_scale = float(l_scale) if l_scale > 1e-8 else 1.0

    # 앞뒤/좌우의 "로컬" 움직임만 남기는 시각화용 오프셋
    x_local = (
        0.030 * np.tanh(a_fwd / f_scale) * (0.45 + 0.55 * swing)
        + 0.010 * np.sin(1.2 * yaw)
    )
    y_local = (
        y_offset
        + 0.040 * np.tanh(a_lat / l_scale) * (0.40 + 0.60 * swing)
        + 0.012 * np.sin(1.6 * yaw)
    )

    x_local = np.clip(moving_savgol(x_local, 31, 3), -cfg.stationary_x_limit, cfg.stationary_x_limit)
    y_center = float(y_offset)
    y_local = moving_savgol(y_local, 31, 3)
    y_local = np.clip(y_local, y_center - cfg.stationary_y_spread / 2, y_center + cfg.stationary_y_spread / 2)
...
def build_sampler(track, t_frames):
    ...
    a_lat_src = track["a_lat"] if "a_lat" in track else np.zeros_like(t)
    a_lat_frames = np.interp(t_frames, t, a_lat_src)
...
def draw_pressure_heatmap(img, rect, sensor_vals, vmax, title, accent, total_pressure, stance_bool):
    ...
    for r in range(3):
        for c in range(3):
            ...
            # ps 라벨은 우측 상단에 작게 배치해 숫자와 시각적으로 분리
            label = f"ps_{idx}"
            ...
            lb_y1 = y1 + 8
            overlay = img.copy()
            cv2.rectangle(overlay, (lb_x1, lb_y1), (lb_x1 + lb_w, lb_y1 + lb_h), (255, 255, 255), -1)
            cv2.addWeighted(overlay, 0.82, img, 0.18, 0, img)
            cv2.rectangle(img, (lb_x1, lb_y1), (lb_x1 + lb_w, lb_y1 + lb_h), LIGHT, 1)
            cv2.putText(img, label, (lb_x1 + 6, lb_y1 + lb_h - 4), cv2.FONT_HERSHEY_SIMPLEX, 0.48, DARK, 1, cv2.LINE_AA)
            txt_color = (255, 255, 255) if val > 0.55 * vmax else BLACK
            val_txt = f"{val:.2f}"
            (vw, vh), _ = cv2.getTextSize(val_txt, cv2.FONT_HERSHEY_SIMPLEX, 0.62, 2)
            cv2.putText(img, val_txt, (x1 + (cell_w - vw) // 2, y1 + cell_h - 18), cv2.FONT_HERSHEY_SIMPLEX, 0.62, txt_color, 2, cv2.LINE_AA)
...
def render_video(left_track, right_track, out_mp4: str | Path, cfg: RenderConfig, mode: str = "forward"):
    ...
    for i, tf in enumerate(t_frames):
        ...
        if mode == "stationary":
            ...
            eye = np.array([-1.15, -1.55, 0.86])
            x_min, x_max = -0.48, 0.48
        ...
        for gx in np.linspace(x_min, x_max, 13 if mode == "stationary" else 16):
            pts = np.array([[gx, y_min, 0.0], [gx, y_max, 0.0]])
            uv, _, _ = project_points(pts, eye, target, motion_w - 20, height - 20, x0=10, y0=10)
            draw_line(img, uv[0], uv[1], LIGHT, 1)
        for gy in np.linspace(y_min, y_max, 9 if mode == "stationary" else 6):
            pts = np.array([[x_min, gy, 0.0], [x_max, gy, 0.0]])
            uv, _, _ = project_points(pts, eye, target, motion_w - 20, height - 20, x0=10, y0=10)
            draw_line(img, uv[0], uv[1], LIGHT, 1)
        if mode == "stationary":
            for gx, col in [(0.0, BLACK)]:
                pts = np.array([[gx, y_min, 0.0], [gx, y_max, 0.0]])
                uv, _, _ = project_points(pts, eye, target, motion_w - 20, height - 20, x0=10, y0=10)
                draw_line(img, uv[0], uv[1], col, 2)
        ...
        if mode == 'stationary':
            cv2.putText(img, 'Stationary view | Local foot motion envelope | Quaternion orientation | Gyro vector | Pressure heatmap',
                        (28, 74), cv2.FONT_HERSHEY_SIMPLEX, 0.60, DARK, 1, cv2.LINE_AA)
            info_lines = [
                f't = {tf:5.2f} s',
                f'LEFT  | stance={int(left_f["stance"][i])} | local_x={left_f["pos"][i,0]:+5.2f} m | local_y={left_f["pos"][i,1]:+5.2f} m | totalP={left_f["pressure"][i]:5.2f}',
                f'RIGHT | stance={int(right_f["stance"][i])} | local_x={right_f["pos"][i,0]:+5.2f} m | local_y={right_f["pos"][i,1]:+5.2f} m | totalP={right_f["pressure"][i]:5.2f}',
            ]
        else:
