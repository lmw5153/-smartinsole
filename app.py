...
def _ground_grid_stationary(x_range=(-0.28, 0.28), y_range=(-0.36, 0.36), z=0.0):
    xs = np.linspace(x_range[0], x_range[1], 11)
    ys = np.linspace(y_range[0], y_range[1], 9)
    traces = []
    for x in xs:
    ...
    traces.append(go.Scatter3d(
        x=[0.0, 0.0], y=[y_range[0], y_range[1]], z=[z, z],
        mode="lines",
...
def make_motion_figure(frame_store: dict, idx: int, cfg: RenderConfig) -> go.Figure:
    ...
    fig.update_layout(
        ...
        height=620,
        scene=dict(
            xaxis=dict(title="Local A/P motion (m)", range=[-0.20, 0.20], backgroundcolor="white", gridcolor="#efefef"),
            yaxis=dict(title="Lateral space (m)", range=[-0.40, 0.40], backgroundcolor="white", gridcolor="#efefef"),
            zaxis=dict(title="Vertical (m)", range=[0.0, 0.13], backgroundcolor="white", gridcolor="#efefef"),
            aspectmode="manual",
            aspectratio=dict(x=1.1, y=1.2, z=0.7),
            camera=dict(
                eye=dict(x=-1.32, y=-1.55, z=0.82),
                center=dict(x=0.0, y=0.0, z=-0.04),
            ),
        ),
        title="제자리 3D: 인솔 자체의 로컬 움직임 · 자세(쿼터니언) · 자이로 벡터",
        showlegend=False,
    )
    return fig
...
def make_pressure_heatmap(sensor_vals: np.ndarray, vmax: float, title: str) -> go.Figure:
    ...
    fig.update_layout(
        ...
        xaxis=dict(showticklabels=False, range=[-0.5, 2.65]),
        yaxis=dict(showticklabels=False, autorange="reversed", range=[2.5, -0.5]),
...
st.caption("전진(절대 위치) 복원은 제거하고, 인솔의 **자세(쿼터니언)** · **회전(자이로)** · **압력 변화**를 앱에서 바로 렌더링합니다. 제자리 뷰에서도 앞뒤/좌우의 로컬 움직임이 보이도록 시각화를 조정했습니다. mp4/ZIP은 필요할 때만 다운로드하세요.")
